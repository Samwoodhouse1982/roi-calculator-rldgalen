 ...
 * Font scale.
 *
 * Originally hard-coded pixel sizes for the 1080×1920 kiosk display.
 * For the embed/responsive build these become fluid values that scale
 * with viewport width using CSS clamp(min, preferred, max):
 ...
 *
 * Each entry is a string ('clamp(...)') rather than a number, so any
 * style prop that wrote `fontSize: F.hero` continues to work - CSS
 * accepts a clamp string the same as a number.
 */
export const F = {
 ...
 *
 * W (1080) is the MAX content width on wide screens. The outer
 * container in App.jsx uses width:100% up to maxWidth:W, so the
 * kiosk's visual identity is preserved on large displays but the
 * layout flows naturally on smaller ones.
 *
 * H (1920) is no longer used as a fixed dimension - the embed build
 * grows naturally with content and uses min-height: 100vh on the
